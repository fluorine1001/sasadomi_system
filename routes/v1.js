...
export default function v1Router(db, admin) {
     ...
     *             properties:
     *               studentId: { type: string, example: "s2026010701", description: "11자리 문자열 학번 기숙사 아이디 (s + 입학년도 4자리 + 학년 2자리 + 반 2자리 + 번호 2자리)" }
     ...
     *               properties:
     *                 success: { type: boolean, example: true }
     *                 studentId: { type: string, example: "s2026010701" }
     *       401:
    ...
    router.get('/points', async (req, res) => {
        ...
        try {
            ...
            // ⭐ 수정 적용: 정확한 ID만 타겟팅하여 불필요한 태그 의존성 제거
     ...
     *             properties:
     *               studentId: { type: string, example: "s2026010701", description: "11자리 문자열 학번 기숙사 아이디 (s + 입학년도 4자리 + 학년 2자리 + 반 2자리 + 번호 2자리). 이 값을 기반으로 원본 서버 폼에 필수적인 grade, class, class_number 파라미터가 내부적으로 파싱되어 함께 전송됩니다." }
     ...
     *       400:
     *         description: 본관 신청 시 지도교사가 누락되었거나 학번 포맷 불일치, 또는 학교 원본 폼 시스템에서 '실패' 스크립트가 반환됨
     *         content:
     *           application/json:
     *             schema:
     *               type: object
    ...
    router.post('/applications/study', async (req, res) => {
        ...
        try {
            ...
            if (studentId && studentId.length === 11 && studentId.startsWith('s')) {
                grade = parseInt(studentId.substring(5, 7), 10).toString();
                sclass = parseInt(studentId.substring(7, 9), 10).toString();
                classNumber = parseInt(studentId.substring(9, 11), 10).toString();
            } else {
                return res.status(400).json({ success: false, message: '올바른 11자리 학번 포맷이 아닙니다.' });
            }
     ...
     *               studentId: { type: string, example: "s2026010701", description: "11자리 문자열 학번 기숙사 아이디 (s + 입학년도 4자리 + 학년 2자리 + 반 2자리 + 번호 2자리). 이 값을 기반으로 원본 서버 폼에 필수적인 grade, class, class_number 파라미터가 내부적으로 파싱되어 함께 전송됩니다." }
     *               token: { type: string }
     *               type: { type: string, enum: [1, 2], description: "1: 외출, 2: 외박", example: "1" }
     *               reason: { type: string, example: "서점 방문 및 도서 구매" }
     *               bdate: { type: integer, description: "한국 표준시(KST) 기준 출발 일시 초 단위 Unix Timestamp", example: 1779897600 }
     *               edate: { type: integer, description: "한국 표준시(KST) 기준 귀교 일시 초 단위 Unix Timestamp", example: 1780066800 }
     *     responses:
     *       200:
     ...
     *                 message: { type: string, example: "완료" }
     *       400:
     *         description: 마감 시간 초과, 학번 포맷 규격 미달 또는 외출 조건 규격 미달로 학교 시스템 측에서 거절됨
     *         content:
     *           application/json:
     *             schema:
     *               type: object
